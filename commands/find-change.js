...
import {
    createPanel, getPanelBody, btn, inputBox, checkRow, iconBtn, setPanelTitleWithBack,
} from '../panel-ui.js';
...
function buildSearchControls() {
    ...
    rangeInput.className = 'ct-range-input';
...
export function runFind(keyword, options = {}) {
    ...
    setPanelTitleWithBack(panel, resultTitleHtml(keyword, count, options), '검색 입력으로 돌아가기', () => {
        clearHighlights();
        panel.remove();
    ...
    const changeBtn = iconBtn('fa-wand-magic-sparkles', '찾아바꾸기로 전환', () => {
        panel.remove();
...
function showChangeResultPanel(find, replaceValue, options) {
    ...
    const body = getPanelBody(panel);
    setPanelTitleWithBack(panel, resultTitleHtml(find, getMarkCount(), options), '검색 입력으로 돌아가기', () => {
        clearHighlights();
        panel.remove();
        openSearchInputPanel();
    });
    ...
    // 왼쪽: 이전/다음 내비게이션. 오른쪽: 바꾸기(현재 매치만)/모두 바꾸기(전체) — 항상 같이 표시됨.
